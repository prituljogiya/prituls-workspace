...
export function Column({ column, tasks, onCreateTask }: ColumnProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="w-[280px] min-w-[280px] max-w-[280px] flex-shrink-0 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg flex flex-col h-full min-h-0 overflow-hidden"
    >
      <div
        {...attributes}
        {...listeners}
        className="px-3 py-2.5 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between cursor-grab active:cursor-grabbing flex-shrink-0"
      >
        <div className="flex items-center gap-2 min-w-0">
          <h3 className="font-semibold text-sm text-gray-700 dark:text-gray-300 uppercase tracking-wide truncate">{column.name}</h3>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden space-y-2 p-2 min-h-0">
      ...
      {showAddTask ? (
        <div className="p-2 border-t border-gray-200 dark:border-gray-700 flex-shrink-0 mt-auto">
          <input
            type="text"
            value={newTaskTitle}
        ...
        canAddTask && (
          <button
            ...
            className="mx-2 mb-2 mt-auto py-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg flex items-center justify-center gap-2 transition-colors flex-shrink-0"
          >
